...
export default function EventForm({ mode }) {
  ...
  useEffect(() => {
    ...
    getEvent(eventId).then((ev) => {
      ...
      setDate(d);
      setTime(t);
      // Older events created before end times existed won't have one --
      // leaving this blank forces filling it in before saving, which is
      // the retroactive fix path for those.
      setEndTime(toDateTimeInputs(ev.endDateTime).time);
      setLoading(false);
    });
  ...
  async function handleSubmit(e) {
    ...
    if (!date || !time) return setError('Pick a start date and time.');
    ...
    const endDateTime = toTimestamp(date, endTime);
    if (endDateTime.toMillis() <= dateTime.toMillis()) {
      return setError('End time needs to be after the start time.');
    }
    ...
    try {
      if (mode === 'create') {
        ...
        await updateEvent(eventId, {
          title, description, location, dateTime, endDateTime,
      ...
      <div className="mx-auto w-full max-w-xl flex-1 px-5 py-10">
        ...
        <form onSubmit={handleSubmit} className="mt-7 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <label className="flex flex-col gap-1.5 text-sm">
              <span className="font-medium text-ink">Start time</span>
            ...
            <label className="flex flex-col gap-1.5 text-sm">
              <span className="font-medium text-ink">End time</span>
