...
export default function EventCard({ event, signupCount, footer }) {
  const past = isPast(event.dateTime);
  const goalMet = typeof signupCount === 'number' && signupCount >= event.volunteersNeeded;
    ...
    <div className={`rounded-card border border-sand-dark bg-paper p-5 ${past ? 'opacity-60' : ''}`}>
      ...
      <div className="mt-4 flex items-center gap-2 text-sm">
        {typeof signupCount === 'number' ? (
          ...
          <span className="text-ink-soft">{event.volunteersNeeded} volunteers needed</span>
        )}
      </div>
